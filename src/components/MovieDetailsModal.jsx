...
const MovieDetailsModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 transition-all duration-300">
      <div className="relative w-full max-w-7xl h-[90vh] bg-zinc-950 rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/5 flex flex-col overflow-hidden">
        
        {/* Absolute Close Button */}
        <button onClick={onClose} className="absolute top-6 right-8 z-50 w-12 h-12 flex items-center justify-center bg-white/5 hover:bg-red-600 text-white rounded-2xl transition-all duration-300 backdrop-blur-md border border-white/10 shadow-2xl">
        ...
        {loading || !movie ? (
          <div className="flex-1 flex items-center justify-center text-white">
            <div className="animate-pulse flex flex-col items-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.4em]">Acquiring Metadata</p>
            </div>
          </div>
        ) : (
            <div className="flex flex-col h-full overflow-y-auto custom-scrollbar bg-[#1a1a1a]">
                {/* Hero Section (BookMyShow Style) */}
                <div className="relative w-full h-[400px] shrink-0 overflow-hidden">
                    {/* Blurred Backdrop */}
                    ...
                    <div className="relative z-10 h-full max-w-6xl mx-auto px-8 flex items-end pb-8 gap-8">
                        {/* Poster */}
                        <div className="w-[200px] h-[300px] rounded-xl overflow-hidden shadow-2xl border border-white/10 self-center">
                            <img 
                        ...
                        <div className="flex-1 space-y-3 pb-2">
                            ...
                            <div className="flex flex-wrap gap-2 py-2">
                                {movie.genres?.map(g => (
                                    <span key={g.id} className="px-3 py-1 bg-white/5 border border-white/10 rounded-md text-[10px] font-bold text-white uppercase">{g.name}</span>
                                ))}
                            </div>
                            ...
                            <div className="flex items-center gap-4 text-white/80 text-xs font-bold uppercase tracking-widest">
                                ...
                                <span className="w-1 h-1 bg-white/20 rounded-full"></span>
                                <span className="px-2 py-0.5 border border-white/20 rounded uppercase text-[8px]">UA</span>
                            </div>

                            <div className="flex gap-4">
                                <button 
                                    onClick={() => onBookTickets(movie)}
                                    className="px-12 py-5 bg-[#f84464] text-white font-black rounded-2xl text-sm uppercase tracking-widest shadow-2xl hover:bg-[#e03a58] transition-all hover:scale-105 active:scale-95"
                                >
                                    Book Tickets
                                </button>
                                {trailerUrl && (
                                    <button 
                                        ...
                                        className="px-8 py-5 bg-white/10 hover:bg-white/20 text-white font-black rounded-2xl text-sm uppercase tracking-widest border border-white/10 transition-all flex items-center gap-3"
                                    >
                ...
                <div className="w-full max-w-6xl mx-auto px-8 py-16 flex flex-col lg:flex-row gap-16">
                    ...
                    <div className="flex-1 space-y-16">
                        ...
                        {/* About */}
                        <section>
                            <h3 className="text-lg font-black text-white uppercase mb-4 flex items-center gap-3">
                                <span className="w-1.5 h-6 bg-[#f84464] rounded-full"></span>
                                About the movie
                            </h3>
                            ...
                            <div className="flex gap-8 overflow-x-auto pb-6 custom-scrollbar scroll-smooth">
                                {movie.credits?.cast.slice(0, 10).map(person => (
                                    <div key={person.id} className="w-24 shrink-0 text-center space-y-3">
                                        <div className="aspect-square rounded-full overflow-hidden border-2 border-transparent hover:border-[#f84464] transition-all grayscale hover:grayscale-0">
                                            <img 
                                        ...
                                        <div>
                                            <p className="text-[10px] font-black text-white uppercase truncate">{person.name}</p>
                                            <p className="text-[8px] font-bold text-gray-500 uppercase truncate mt-0.5 italic">{person.character}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex gap-8 overflow-x-auto pb-6 custom-scrollbar scroll-smooth">
                                {movie.credits?.crew.filter(c => ['Director', 'Writer', 'Producer'].includes(c.job)).slice(0, 8).map(person => (
                                    <div key={person.id + person.job} className="w-24 shrink-0 text-center space-y-3">
                                        <div className="aspect-square rounded-full overflow-hidden border-2 border-transparent bg-white/5 flex items-center justify-center">
                                            {person.profile_path ? (
                                        ...
                                        <div>
                                            <p className="text-[10px] font-black text-white uppercase truncate">{person.name}</p>
                                            <p className="text-[8px] font-bold text-gray-500 uppercase truncate mt-0.5 italic">{person.job}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                    ...
                                    >
                                        <div className="aspect-[2/3] rounded-xl overflow-hidden mb-3 relative">
                                            ...
                                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                <button className="px-4 py-2 bg-white text-black text-[10px] font-black uppercase rounded-lg">View Details</button>
                                            </div>
                                        </div>
                                        <p className="text-[10px] font-black text-white uppercase truncate">{m.title}</p>
                                        <p className="text-[8px] font-bold text-gray-500 uppercase">{m.release_date?.split('-')[0]}</p>
                                    </div>
                                ))}
                            </div>
                        </section>
